...
for key in Arbeitsdaten:
    ...
    for i in range(0,(int(len(Arbeitsdaten[key])/5))):# ein durchlauf pro Block der jeweiligen Kraft/ des Moments (jeder Block hat 5 einträge in "Arbeitsdaten")
        ...
        esp = asp + laenge #int(Arbeitsdaten[key][4 + i * 5])#endspalte
        einzelblock = pd.concat([einzelblock, df[1]]).iloc[az : ez , asp : esp ]], axis = 1)# wenn mehr als ein Block vorhanden werden diese verknüpft
    ...
    Datenbloecke[key] = einzelblock# einzelne Bloecke werden auf OrderedDict geschrieben
Lastpunkt_dict = {}

if __name__ == '__main__':
    indz = 3# indexe der Zeilen für zähnr, slices und Lastpunkt
    inds = 5
    indl = 8
    ...
    Motordrehzahl = df_dict[df_keys[int(Kerndaten['Lastpunkt'][0])]].iloc[int(Kerndaten['Lastpunkt'][1]) - 1][int(Kerndaten['Lastpunkt'][2]) - 1]
    ...
    Motordrehzahl = int(erweitertes_Auslesen(Motordrehzahl, Kerndaten['Lastpunkt'][3]))     
...
for i in range(2,Auswahl.datei_zahl+1):# testetb ob daten für zähne, slices und Winkelauflösung voneinander abweichen
    testz= df_dict[i].iloc[indz][1]         
    tests = df_dict[i].iloc[inds][1]   
    testw = float(df_dict[i].iloc[20][0]) - float(df_dict[i].iloc[19][0])
    
    Lastpunkt_dict[i]= df_dict[i].iloc[indl][1]# erstellt dictionaray mit den Lastpunkten
